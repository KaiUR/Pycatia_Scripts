'''
    ...
    Date:           27.01.26
    Description:    This script will ask the user for an axis system and for a geometric set containing points.
                    The script will measure the points relative to the axis system that is selected and the create an
                    xyz file. The name of the points in the exported file are the same as in catia for each point. The axis system used
                    in this script cannot be in a geometric set, it must be in the axis systems collection. This scrip assumes that the 
                    points are explicit (Isolated)
    dependencies = [
                    "pycatia",
                    ]
    -----------------------------------------------------------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    axis_id = 0                                                                                                 #ABS axix systme as default
    if status == "Normal":                                                                                      #Check if selection was succesful
        found = False                                                                                           #Set found flag
        for chk in range(axis_systems.count):                                                                   #Cycle through all axis systems
            if part_document.selection.item(1).value.name == axis_systems.item(chk + 1).name:                   #If found selected axis systme
    ...
    part_document.selection.clear()                                                                             #Clear user selection
    ...
    print("\n Creating file\n\n")
    f = open("output.xyz", "w")                                                                                 #Create new file
    '''
        XYZIJK METRIC refers to a 3D measurement file format used in metrology (often PC-DMIS) defining 
        a point's position (\(X,Y,Z\)) and normal vector (\(I,J,K\)) using millimeter units. This format 
        is critical for CMM inspection, defining both where to measure and the probe's orientation relative 
        to the surface.
    '''
    f.write("XYZIJK     METRIC\n")                                                                              #xyz file header
