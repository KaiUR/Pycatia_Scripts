'''
    ...
    Date:           21.05.26
    Description:    Creates a new CATDrawing document and populates its background view with a
                    standard ISO 5457 border and an ISO 7200-inspired title block.
                    Two dialogs are shown: the first to select the paper size (A0–A4),
                    the second to enter values for all title block fields (Company, Title,
                    Part Number, Material, Scale, Sheet, Revision, Drawn By, Date, Approved).
                    Entered values are written directly into the named text items so the drawing
                    is ready to use immediately. Fields left blank remain empty.
    dependencies = [
                    "pycatia",
                    "wxPython",
                    ]
...
class PaperDialog(wx.Dialog):
    def __init__(self, parent):
        super().__init__(parent, title="Create Drawing — Step 1: Paper Size",
                         size=(380, 160), style=wx.DEFAULT_DIALOG_STYLE | wx.STAY_ON_TOP)
        panel      = wx.Panel(self)
        panel_sizer = wx.BoxSizer(wx.VERTICAL)

        panel_sizer.Add(wx.StaticText(panel, label="Select paper size:"), flag=wx.ALL, border=12)
        self.choice = wx.Choice(panel, choices=[o[0] for o in PAPER_OPTIONS])
        self.choice.SetSelection(3)                                                                                  #Default: A3 Landscape
        panel_sizer.Add(self.choice, flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.BOTTOM, border=12)
    ...
    def get_paper(self):
        return PAPER_OPTIONS[self.choice.GetSelection()]


class ValuesDialog(wx.Dialog):
    def __init__(self, parent):
        super().__init__(parent, title="Create Drawing — Step 2: Title Block Values",
...
if __name__ == "__main__":
    ...
    dlg1 = PaperDialog(None)
    wx.CallAfter(_bring_to_front, dlg1)
    if dlg1.ShowModal() != wx.ID_OK:
        dlg1.Destroy()
        print("Cancelled.")
        exit()
    paper_name, cat_size, cat_orient = dlg1.get_paper()
    dlg1.Destroy()

    dlg2 = ValuesDialog(None)
    wx.CallAfter(_bring_to_front, dlg2)
    if dlg2.ShowModal() != wx.ID_OK:
        dlg2.Destroy()
        print("Cancelled.")
        exit()
    values = dlg2.get_values()
    dlg2.Destroy()
    ...
    print(f"  Border and title block created  ({filled} of {len(values)} fields populated).")
