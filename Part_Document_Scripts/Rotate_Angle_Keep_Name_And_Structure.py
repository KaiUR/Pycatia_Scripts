'''
    -----------------------------------------------------------------------------------------------------------------------
    Script name:    Rotate_Angle_Keep_Name_And_Structure.py
    Version:        1.1
    Code:           Python3.10.4, Pycatia 0.8.3
    Release:        V5R32
    Purpose:        Rotates all hybrid shapes in a geometric set by an angle while keeping names and structure.
    Author:         Kai-Uwe Rathjen
    Date:           12.05.26
    Description:    This script will ask the user to select a geometric set, a rotation axis and an angle.
                    The script will recreate the full geometric set structure inside the current in-work object,
                    perform a rotation on every hybrid shape recursively through all child sets, and preserve
    ...
    requirements:   Python >= 3.10
                    pycatia
                    wxPython
                    Catia V5 running with an open part containing a geometric set and a line.
...
def create_datum(hybrid_shape_factory, hybrid_shape, hybrid_body, name=None):
    ...
    hybrid_shape_factory.delete_object_for_datum(hybrid_shape)                                                  #Remove original rotate shape

'''
    This function recursively processes a source geometric set, recreating its structure in the target
    geometric set and performing a rotation on every hybrid shape.
    ...
    Inputs:
        ...
        hybrid_shape_factory    The hybrid shape factory for the part
        axis_ref                Reference to the rotation axis
        angle                   The rotation angle in degrees

    output:
        None
'''
def process_hybrid_body(source_hb, target_hb, part, hybrid_shape_factory, axis_ref, angle):
    ...
    for index in range(hybrid_shapes.count):                                                                    #Loop through all shapes in source set
        ...
        create_datum(hybrid_shape_factory, rotate, target_hb, shape_name)                                      #Convert to datum preserving name
...
if __name__ == "__main__":
    ...
    dlg = wx.TextEntryDialog(None, "Enter angle to rotate (degrees):", "Enter Angle", "0.0",
    ...
    if inwork_hb is None:                                                                                      #If still not found, create new GS
        ...
        inwork_hb.name = "Rotate_Angle_Keep_Name_And_Structure"                                                 #Rename geometric set
