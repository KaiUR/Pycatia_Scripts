'''
    -----------------------------------------------------------------------------------------------------------------------
    Script name:    Mirror_Keep_Name_And_Structure.py
    Version:        1.4
    Code:           Python3.10.4, Pycatia 0.8.3
    Release:        V5R32
    Purpose:        Mirrors all hybrid shapes in a geometric set while keeping names and structure.
    Author:         Kai-Uwe Rathjen
    Date:           12.05.26
    Description:    This script will ask the user to select a geometric set and a mirror plane.
                    The script will recreate the full geometric set structure inside the current in-work object,
                    perform a mirror (symmetry) on every hybrid shape recursively through all child sets, and
                    preserve the original names of all shapes and geometric sets.
    dependencies = [
                    "pycatia",
                    ]
    requirements:   Python >= 3.10
                    pycatia
                    Catia V5 running with an open part containing a geometric set and a plane.
    ...
    Change:         13.05.26 1.1: Replace name-based HybridBody lookup with direct COM reference.
                    ...
                    16.05.26 1.3: Guard against in-work object being selected as source geometric set.
...
def create_datum(hybrid_shape_factory, hybrid_shape, hybrid_body, name=None):
    ...
    hybrid_shape_factory.delete_object_for_datum(hybrid_shape)                                                  #Remove original symmetry shape

'''
    This function recursively processes a source geometric set, recreating its structure in the target
    geometric set and performing a mirror (symmetry) on every hybrid shape.
    ...
    Inputs:
        ...
        hybrid_shape_factory    The hybrid shape factory for the part
        mirror_ref              Reference to the mirror plane

    output:
        None
'''
def process_hybrid_body(source_hb, target_hb, part, hybrid_shape_factory, mirror_ref):
    ...
    for index in range(hybrid_shapes.count):                                                                    #Loop through all shapes in source set
        ...
        symmetry = hybrid_shape_factory.add_new_symmetry(shape_ref, mirror_ref)                                 #Create mirror (symmetry)
        symmetry.name = shape_name                                                                              #Set name to match source shape
        target_hb.append_hybrid_shape(symmetry)                                                                 #Add to target geometric set
        part.update()                                                                                           #Update part

        create_datum(hybrid_shape_factory, symmetry, target_hb, shape_name)                                     #Convert to datum preserving name
    ...
    for child_index in range(source_hb.hybrid_bodies.count):                                                    #Loop through child geometric sets in source
        ...
        process_hybrid_body(source_child_hb, target_child_hb, part,                                            #Recurse into child set
                hybrid_shape_factory, mirror_ref)
...
if __name__ == "__main__":
    ...
    status = selectionSet.select_element3(object_filter, "Select geometric set to mirror", False, 2, False)    #Runs an interactive selection command, exhaustive version.
    ...
    if inwork_hb is None:                                                                                      #If still not found, create new GS
        ...
        inwork_hb.name = "Mirror_Keep_Name_And_Structure"                                                       #Rename geometric set
