...
| `Manage_Program_Names_And_Comments.py` | Review the machining tree and set program and operation names and comments from template lists; metal, master and spotting per part operation, program names built and renumbered per part operation, comments composed with the machine offset the master and metal rule gives shown in its own M/C column, PP instructions set on PPInstruction activities and placed by selection, whole part operations named and commented at once with the stage taken from each divider, a program's stage propagated down to its operations, PP instructions added blank or fully defined and removed, placed by selection, a menu-bar button row, several rows editable together as a group, and stepover, depth of cut and part offset checked against editable limits with values that are off shown on red |
