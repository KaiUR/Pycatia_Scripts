'''
    ...
    Release:        V5R32
    Purpose:        Toggle the visibility of a selected geometric set and all its children.
    Author:         Kai-Uwe Rathjen
    Date:           06.05.26
    Description:    This script will ask the user to select a geometric set. The script will then toggle the visibility
                    of the geometric set and all children inside it. If the set is currently visible it will be hidden,
...
'''
    ...
    Inputs:
        ...
        new_show        The visibility state to apply (0 = shown, 1 = hidden)
...
if __name__ == "__main__":
    ...
    if current_show == 0:                                                                                       #If currently visible
        new_show = 1                                                                                            #Set to hide
        print(f"Hiding geometric set: {selected_hb.name}")
    else:                                                                                                       #If currently hidden
        new_show = 0                                                                                            #Set to show
        print(f"Showing geometric set: {selected_hb.name}")

    selectionSet.clear()                                                                                        #Clear selection
    selectionSet.add(selected_hb)                                                                               #Add geometric set to selection
    selectionSet.vis_properties.set_show(new_show)                                                              #Apply visibility to the geometric set itself

    toggle_visibility_recursive(selected_hb, selectionSet)                                                     #Recursively toggle visibility of all children
